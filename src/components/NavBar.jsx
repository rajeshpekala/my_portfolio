  ...
  return (
    ...
    <div>
     ...
     <ul className='hidden md:flex'>
        ...
        {links.map((link) => (
         ...
         <li 
         key = {link.id} className='px-4 cursor-pointer capitalize 
   ...
   {nav && (
    ...
    {links.map((link) => (
      ...
      <li 
      key = {link.id} className='py-4 cursor-pointer capitalize 
      font-medium text-gray-500 hover:scale-110 duration-200 text-4xl '
      >
        <Link to = {link.link} 
        onClick={()=> SetNav(!nav)}
        smooth duration ={500}>{link.link}</Link>
